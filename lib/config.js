const config = {
	...
	gistDescription: {
		...
		type: 'string',
		default: 'Atom Settings Backup by https://atom.io/packages/sync-settings-fork',
	},
	useOtherLocation: {
...
function updateLegacyConfigSettings () {
	if (typeof atom.config.get('sync-settings-fork.warnBackupConfig') !== 'undefined') {
		atom.config.set('sync-settings-fork.hiddenSettings._warnBackupConfig', atom.config.get('sync-settings-fork.warnBackupConfig'))
		atom.config.unset('sync-settings-fork.warnBackupConfig')
	}
}

function getLastBackupTime (data, saveLast) {
	const lastBackupTime = atom.config.get('sync-settings-fork.hiddenSettings._lastBackupTime')

	if (saveLast && lastBackupTime !== data.time) {
		atom.config.set('sync-settings-fork.hiddenSettings._lastBackupTime', data.time)
		atom.config.unset('sync-settings-fork._lastBackupHash')
		atom.config.unset('sync-settings-fork.hiddenSettings._lastBackupHash')
		return data.time
	}
	...
	const lastHash = atom.config.get('sync-settings-fork._lastBackupHash') || atom.config.get('sync-settings-fork.hiddenSettings._lastBackupHash')
	if (!lastHash) {
		return lastBackupTime
	}
	...
	if (data.history) {
		for (const history of data.history) {
			if (history.version === lastHash) {
				atom.config.set('sync-settings-fork.hiddenSettings._lastBackupTime', history.committed_at)
