...
module.exports = class SyncSettings {
	constructor () {
		...
		if (atom.config.get('sync-settings-fork.checkForUpdatedBackup')) {
	...
	async getBackupLocation (waitForLocationService) {
		if (!atom.config.get('sync-settings-fork.useOtherLocation')) {
	...
	async checkBackup (autoCheck) {
		...
		try {
			...
			let hideVersionsUpdate = atom.config.get('sync-settings-fork.installLatestVersion')
	...
	async backup () {
		...
		try {
			...
			if (atom.config.get('sync-settings-fork.removeUnfamiliarFiles')) {
				const data = await backupLocation.get()
				if (!data) {
					return
				}
			...
			atom.config.set('sync-settings-fork.hiddenSettings._lastBackupTime', data.time)
	...
	async restore () {
		...
		try {
			...
			if (backupData.packages) {
				...
				if (atom.config.get('sync-settings-fork.removeObsoletePackages')) {
	...
	async getLocalData () {
		...
		if (atom.config.get('sync-settings-fork.syncSettings')) {
			data.settings = utils.getFilteredSettings()
		}
		if (atom.config.get('sync-settings-fork.syncPackages') || atom.config.get('sync-settings-fork.syncThemes')) {
			data.packages = await utils.getPackages()
		}
		const removeUnfamiliarFiles = atom.config.get('sync-settings-fork.removeUnfamiliarFiles')
		if (atom.config.get('sync-settings-fork.syncKeymap')) {
		...
		if (atom.config.get('sync-settings-fork.syncStyles')) {
		...
		if (atom.config.get('sync-settings-fork.syncInit')) {
		...
		if (atom.config.get('sync-settings-fork.syncSnippets')) {
		...
		const extraFiles = atom.config.get('sync-settings-fork.extraFiles') || []
		...
		const extraFilesGlob = atom.config.get('sync-settings-fork.extraFilesGlob') || []
		const ignoreFilesGlob = atom.config.get('sync-settings-fork.ignoreFilesGlob') || []
				...
				switch (fileName) {
					...
					case 'packages.json':
						if (atom.config.get('sync-settings-fork.syncPackages') || atom.config.get('sync-settings-fork.syncThemes')) {
							...
							if (!atom.config.get('sync-settings-fork.syncPackages')) {
								data.packages = utils.filterObject(data.packages, ([k, v]) => v.theme)
							}
							if (!atom.config.get('sync-settings-fork.syncThemes')) {
								data.packages = utils.filterObject(data.packages, ([k, v]) => !v.theme)
							}
							if (atom.config.get('sync-settings-fork.onlySyncCommunityPackages')) {
					...
					default: {
						...
						let extraFiles = atom.config.get('sync-settings-fork.extraFiles') || []
