...
const REMOVE_KEYS = [
	'sync-settings-fork.gistId',
	'sync-settings-fork.personalAccessToken',
	'sync-settings-fork.hiddenSettings._lastBackupTime',
	// keep legacy keys in blacklist
	'sync-settings-fork.gistId',
	'sync-settings-fork.personalAccessToken',
	'sync-settings-fork._analyticsUserId',
	'sync-settings-fork._lastBackupHash',
	'sync-settings-fork.hiddenSettings._lastBackupHash',
]
...
module.exports = {
	...
	async addExtraFile (files, file, removeUnfamiliarFiles) {
		...
		if (file === 'config.cson' && atom.config.get('sync-settings-fork.personalAccessToken') && atom.config.get('sync-settings-fork.hiddenSettings._warnBackupConfig')) {
	...
	async getPackages () {
		const syncPackages = atom.config.get('sync-settings-fork.syncPackages')
		const syncThemes = atom.config.get('sync-settings-fork.syncThemes')
		const onlySyncCommunityPackages = atom.config.get('sync-settings-fork.onlySyncCommunityPackages')
		const packages = {}
	...
	addFilteredSettings (settings) {
		const blacklistedKeys = [
			...REMOVE_KEYS,
			...atom.config.get('sync-settings-fork.blacklistedKeys') || [],
		]
	...
	async installPackage (pkg) {
		...
		await new Promise((resolve, reject) => {
			if (atom.config.get('sync-settings-fork.installLatestVersion')) {
				pkg.version = null
