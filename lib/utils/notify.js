...
Do you want to back up this file anyway?`.trim(),
			...
			buttons: [{
				...
				onDidClick () {
					...
					atom.commands.dispatch(atom.views.getView(atom.workspace), 'sync-settings-fork:backup')
				},
			}],
		})
	},
	...
	newerBackup (autoCheck, lastBackupTime, backupTime) {
		...
		const buttons = [{
			...
			onDidClick () {
				notification.dismiss()
				atom.commands.dispatch(workspaceElement, 'sync-settings-fork:restore')
			},
		}, {
			text: 'Backup',
			onDidClick () {
				notification.dismiss()
				atom.commands.dispatch(workspaceElement, 'sync-settings-fork:backup')
			},
		}, {
			text: 'View Backup',
			onDidClick () {
				atom.commands.dispatch(workspaceElement, 'sync-settings-fork:view-backup')
			},
		}, {
			text: 'View Diff',
			onDidClick () {
				notification.dismiss()
				atom.commands.dispatch(workspaceElement, 'sync-settings-fork:view-diff')
			},
		}]
	...
	newBackup (action) {
		notify.success(`Sync-Settings: ${action} successfully`, {
			...
			buttons: [{
				...
				text: 'View Backup',
				onDidClick () {
					atom.commands.dispatch(atom.views.getView(atom.workspace), 'sync-settings-fork:view-backup')
				},
			}],
		})
	},
