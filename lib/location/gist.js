	...
	const notification = notify.error(invalidId ? 'Invalid Gist ID' : 'No Gist ID', {
		...
		buttons: [{
			...
			async onDidClick () {
				...
				if (gistId) {
					atom.config.set('sync-settings-fork.gistId', gistId)
					resolveFn(gistId)
				}
				rejectFn()
			},
			...
			async onDidClick () {
				...
				notification.dismiss()
				try {
					await atom.commands.dispatch(atom.views.getView(atom.workspace), 'sync-settings-fork:create-backup')
					if (atom.config.get('sync-settings-fork.gistId')) {
						resolveFn(atom.config.get('sync-settings-fork.gistId'))
					}
				} catch (ex) {}
				rejectFn()
			},
			...
			onDidClick () {
				notification.dismiss()
				atom.workspace.open('atom://config/packages/sync-settings-fork')
			},
		}],
	})
...
async function getGistId (allowEmpty) {
	let gistId = atom.config.get('sync-settings-fork.gistId') || process.env.GIST_ID
	if (gistId) {
		return gistId.trim()
	}
...
and set the environment variable \`GITHUB_TOKEN\` or enter it in the settings.`.trim(),
		...
		buttons: [{
			...
			async onDidClick () {
				...
				if (personalAccessToken) {
					atom.config.set('sync-settings-fork.personalAccessToken', personalAccessToken)
...
async function getPersonalAccessToken (allowEmpty) {
	let token = atom.config.get('sync-settings-fork.personalAccessToken') || process.env.GITHUB_TOKEN
	if (token) {
		return token.trim()
	}
...
module.exports = {
	...
	async create () {
		...
		try {
			...
			atom.config.set('sync-settings-fork.gistId', res.data.id)
	...
	async delete () {
		...
		try {
			...
			atom.config.unset('sync-settings-fork.gistId')
	...
	async update (buffers) {
		...
		try {
			...
			const res = await createClient(personalAccessToken).gists.update({
				gist_id: gistId,
				description: atom.config.get('sync-settings-fork.gistDescription'),
				files,
			})
